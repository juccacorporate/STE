      ...
      fetch(API_URL)
        ...
        .then(data => {
          const formatData = data.map((item: any) => ({
            ...
            // Nomes das colunas exatamente como na sua planilha
          ...
          <div className="rounded-[2.5rem] border border-white/10 bg-white/5 p-10 backdrop-blur-xl shadow-2xl">
            <form onSubmit={handleLogin} className="space-y-6">
              <input
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-center text-white outline-none focus:border-indigo-500"
                autoFocus
              />
    ...
    <div className="flex flex-col min-h-screen bg-[#FDFDFF]">
      <header className="bg-white border-b border-gray-100 p-4 sticky top-0 z-30 shadow-sm">
        ...
        <div className="max-w-[1600px] mx-auto mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
            <select value={filters.region} onChange={(e) => setFilters(f => ({ ...f, region: e.target.value as any }))} className="text-[11px] border border-gray-200 rounded-xl p-2.5 bg-white font-bold"><option value="Todos">{t.region}: {t.all}</option>{Object.values(Region).map(r => <option key={r} value={r}>{r}</option>)}</select>
            <select value={filters.priority} onChange={(e) => setFilters(f => ({ ...f, priority: e.target.value as any }))} className="text-[11px] border border-gray-200 rounded-xl p-2.5 bg-white font-bold"><option value="Todos">{t.priority}: {t.all}</option>{Object.entries(t.priorities).map(([key, val]) => <option key={key} value={key}>{val}</option>)}</select>
            <select value={filters.owner} onChange={(e) => setFilters(f => ({ ...f, owner: e.target.value as any }))} className="text-[11px] border border-gray-200 rounded-xl p-2.5 bg-white font-bold"><option value="Todos">{t.owner}: {t.all}</option>{ownersList.map(o => <option key={o} value={o}>{o}</option>)}</select>
            <select value={String(filters.delayed)} onChange={(e) => { const val = e.target.value; setFilters(f => ({ ...f, delayed: val === 'Todos' ? 'Todos' : val === 'true' })) }} className="text-[11px] border border-gray-200 rounded-xl p-2.5 bg-white font-bold"><option value="Todos">Prazo: {t.all}</option><option value="true">No Prazo</option><option value="false">Atrasados</option></select>
            <select value={filters.status} onChange={(e) => setFilters(f => ({ ...f, status: e.target.value as any }))} className="text-[11px] border border-gray-200 rounded-xl p-2.5 bg-white font-bold"><option value="Todos">Status: {t.all}</option>{Object.entries(t.statuses).map(([key, val]) => <option key={key} value={key}>{val}</option>)}</select>
